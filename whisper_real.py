...
root = tk.Tk()
root.title("Whisperization")
root.geometry("600x300")


tk.Label(root, text="").pack()

# Create a scale widget for the variable y2
y2_var = tk.DoubleVar()
y2_scale = tk.Scale(root, variable=y2_var, from_=0.1, to=0.5, resolution=0.01, orient=tk.HORIZONTAL, label="Whisperization loudness", width=20, font=("Helvetica", 20))
y2_scale.pack(fill=tk.X)

# Add a blank line between the two scales
tk.Label(root, text="").pack()

# Create a scale widget for the variable threshold
threshold_var = tk.DoubleVar()
threshold_scale = tk.Scale(root, variable=threshold_var, from_=0.01, to=0.02, resolution=0.001, orient=tk.HORIZONTAL, label="Nosie threshold", width=20, font=("Helvetica", 20))
...
while is_running:
    ...
    y2 *= y2_var.get()  # Apply the scaling factor from the slider
